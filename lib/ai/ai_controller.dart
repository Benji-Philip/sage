...
class AiController {
  ...
  Future<String> generateSuggestedQuestions(
      WidgetRef ref, String patientSex, double patientAge, int patientAgeUnit, String patientOccupation, String patientHopi, String patientExaminations) async {
        String aiModel = ref.read(settingsDatabaseProvider)[2].value;
    String prompt = "You're part of an educational app that helps medical students understand different cases. A ${patientSex == "M"? "male":patientSex == "F"? "female":"non-binary"} patient of age $patientAge ${patientAgeUnit == 0 ? "years" : patientAgeUnit == 1 ? "months":patientAgeUnit == 2 ? "days":patientAgeUnit == 3 ? "hours" : ""}${patientOccupation.isEmpty?"":patientOccupation.trim() == "" ? "":" who is a $patientOccupation"}, with the following history: $patientHopi${patientExaminations.isNotEmpty && patientExaminations.trim() != "" ? " and examinations: $patientExaminations":""}. List 3 to 5 recommended questions that they should ask with the current information, few to rule out organ systems or diseases not involved and few to help obtain patient history relevant to the most probable diagnoses. Don't give any explanations. Don't use any headings. In plain text not markdown";
    if (aiModel == "gemini") {
      String response = await GeminiApi().promptGeminiText(prompt, ref);
      return response;
    } else if (aiModel == "chatgpt"){
  ...
  Future<String> generateTreatments(
      WidgetRef ref, String patientSex, double patientAge, int patientAgeUnit, String patientOccupation, String patientHopi, String patientExaminations) async {
        String aiModel = ref.read(settingsDatabaseProvider)[2].value;
    String prompt = "You're part of an educational app that helps medical students understand different cases. A ${patientSex == "M"? "male":patientSex == "F"? "female":"non-binary"} patient of age $patientAge ${patientAgeUnit == 0 ? "years" : patientAgeUnit == 1 ? "months":patientAgeUnit == 2 ? "days":patientAgeUnit == 3 ? "hours" : ""}${patientOccupation.isEmpty?"":patientOccupation.trim() == "" ? "":" who is a $patientOccupation"}, with the following history: $patientHopi${patientExaminations.isNotEmpty && patientExaminations.trim() != "" ? " and examinations: $patientExaminations":""}. List most probable treatments, management, or surgery including the name of the drugs class, methods, techniques, or surgeries, for the most likely diagnosis. Make it short. Don't use any headings. In points in plain text not markdown";
    if (aiModel == "gemini") {
      String response = await GeminiApi().promptGeminiText(prompt, ref);
      return response;
    } else if (aiModel == "chatgpt"){
  ...
  Future<String> refactorHistory(
      WidgetRef ref, String patientHopi) async {
        String aiModel = ref.read(settingsDatabaseProvider)[2].value;
    String prompt = "The history for a patient is as follows : $patientHopi. Refactor the given patient history under seperate headings like you would see in hospital casesheets, under points in plain text and not in markdown. Only refactor the given history into what doctors would write in patient casesheets";
    if (aiModel == "gemini") {
      String response = await GeminiApi().promptGeminiText(prompt, ref);
      return response;
    } else if (aiModel == "chatgpt"){
  ...
  Future<String> generateSummary(
      WidgetRef ref, String patientSex, double patientAge, int patientAgeUnit, String patientOccupation, String patientHopi, String patientExaminations) async {
        String aiModel = ref.read(settingsDatabaseProvider)[2].value;
    String prompt = "You're part of an educational app that helps medical students understand different cases. A ${patientSex == "M"? "male":patientSex == "F"? "female":"non-binary"} patient of age $patientAge ${patientAgeUnit == 0 ? "years" : patientAgeUnit == 1 ? "months":patientAgeUnit == 2 ? "days":patientAgeUnit == 3 ? "hours" : ""} with the following history: $patientHopi${patientExaminations.isNotEmpty && patientExaminations.trim() != "" ? " and examinations: $patientExaminations":""}. Summarise this case into a singular paragraph which includes patient details, the positive findings from the history only and the most probable diagnosis. Don't give any explanations. Don't use any headings. In plain text not markdown";
    if (aiModel == "gemini") {
      String response = await GeminiApi().promptGeminiText(prompt, ref);
      return response;
    } else if (aiModel == "chatgpt"){
